...
def spamTest():
    ...
    fullText = []
    for i in range(1, 26):
        wordList = textParse(open('E:\Datas\PycharmProjects\MLAction\dataset\_naiveBayes_email\spam\%d.txt' % i).read())  # 读取并解析垃圾邮件
        """注意append和extend的区别，循环完成后，docList是一个2维列表，而fullText是一个1维列表"""
        docList.append(wordList)
        fullText.extend(wordList)
        classList.append(1)
        wordList = textParse(open('E:\Datas\PycharmProjects\MLAction\dataset\_naiveBayes_email\ham\%d.txt' % i).read())
        docList.append(wordList)
        fullText.extend(wordList)
        classList.append(0)
...
if __name__ == '__main__':
    ...
    for i in range(0, 10):
        ...
        aveErrorRate += errorRate[i]
    print(errorRate)
    print('平均错误率是：%s %%' % (aveErrorRate*100/len(errorRate)))