...
def spamTest():
    ...
    fullText = []
    for i in range(1, 26):
        wordList = textParse(open('..\dataset\_naiveBayes_email\spam\%d.txt' % i).read())  # 读取并解析垃圾邮件
        """注意append和extend的区别，循环完成后，docList是一个2维列表，而fullText是一个1维列表"""
        docList.append(wordList)
        fullText.extend(wordList)
        classList.append(1)
        wordList = textParse(loadD.loadDataSets('..\dataset\_naiveBayes_email\ham\%d.txt' % i).read())
        docList.append(wordList)
        fullText.extend(wordList)
        classList.append(0)
